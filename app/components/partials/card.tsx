...
const ServiceCard = ({title, subtitle, description,icon}: {title: string, subtitle: string, description: string, icon: string }) => {
  ...
  const changeIcon = (icons:string) => {
      switch (icons) {
        ...
        break;
        case "course" : return <FaPhotoVideo size={30} className='fill-neutral-800'/>
        break;
      }
  ...
  return (
    <div className="card bg-white border-neutral-300 border-[1.2px] rounded-2xl p-8">
    <div className="icons">
      {changeIcon(icon)}
    </div>
    <div className="middle-text mt-2">
        <p className="font-bold text-xl text-neutral-700">{title}</p>
